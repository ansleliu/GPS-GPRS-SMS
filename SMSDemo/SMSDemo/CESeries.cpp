...
*/
BOOL CCESeries::OpenPort(CWnd* pOwner,
						 UINT portNo	,			/*串口号*/
						 UINT baud		,			/*波特率*/
						 UINT parity	,			/*奇偶校验*/
						 UINT databits	,			/*数据位*/
...
/*写内容进串口*/
BOOL CCESeries::WriteChar(char *buf, int nsize)
{
	DWORD dwNumBytesWritten;
	BOOL bResult;
	bResult = WriteFile(m_hComm,buf,nsize,&dwNumBytesWritten,NULL);
...
/*写内容进串口*/
BOOL CCESeries::WriteChar(CString str)
{
	DWORD dwNumBytesWritten;
	BOOL bResult;
	int len=str.GetLength();
	BYTE *psendbuf=new BYTE[len];
	for(int i = 0; i < len;i++)
		psendbuf[i] = (BYTE)str.GetAt(i);// 转换为单字节字符
	bResult = WriteFile(m_hComm,psendbuf,len,&dwNumBytesWritten,NULL);
	if(!bResult || dwNumBytesWritten == 0) return FALSE;
	return TRUE;
}
/*写内容进串口*/
BOOL CCESeries::WriteChar(LPCTSTR lpString)
{
	DWORD dwNumBytesWritten;
	BOOL bResult;
	bResult = WriteFile(m_hComm,lpString,wcslen(lpString)*2,&dwNumBytesWritten,	NULL);
	if(!bResult || dwNumBytesWritten == 0) return FALSE;
	return TRUE;
}
